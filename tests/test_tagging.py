...
def test_add_tag_empty_repo(click_runner):
    ...
    result = click_runner.invoke(git_semver, ["-T", "-e", "7.7.7"])

    assert result.exit_code == 0
    assert result.stderr is ''
    assert pf("7.7.7") in result.stdout
    assert str(r.tags[-1]) == pf("7.7.7")


def test_add_tag_repo_with_tags(click_runner):
    r = git_repo(Path.cwd())
...
def test_push_tag(click_runner, tmp_path):
    ...
    result = click_runner.invoke(git_semver, ["-TU", "-e", "7.7.7"])

    assert result.exit_code == 0
    assert result.stderr is ''
    assert [str(t) for t in remote.tags if str(t).startswith(DEFAULT_PREFIX)] == [pf("1.1.1"), pf("1.1.2"), pf("7.7.7")]
